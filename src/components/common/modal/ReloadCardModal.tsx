...
const useReloadCardModal = () => {
  const [isOpen, setIsOpen] = useState(false);
  const [title, setTitle] = useState('');
  const backdrop = 'blur';
  const route = useRouter();
  const [user, setUser] = useRecoilState(userState);
  const openReloadCardModal = (newTitle: string) => {
    setTitle(newTitle);
    setIsOpen(true);
  };

  const closeReloadCardtModal = () => {
    setIsOpen(false);
  };

  const reloadCardModal = () => (
    <Modal
      className="w-[20rem] text-gray-500"
      placement="center"
      backdrop={backdrop as ModalProps['backdrop']}
      isOpen={isOpen}
      onClose={closeReloadCardtModal}
    >
      <ModalContent>
        {(onClose) => (
          <div>
  ...
  return { openReloadCardModal, closeReloadCardtModal, reloadCardModal };
};

export default useReloadCardModal;
