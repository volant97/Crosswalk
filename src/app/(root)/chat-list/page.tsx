...
// TODO: UI 로직 & 비즈니스 로직 -> 분리
export default function ChatListPage() {
  const { chatList, lastMsg } = useChatData()
  const getUid = useRecoilValue(userState);
  const router = useRouter();
  const { openModal, AlertModal } = useAlertModal();
  const { openModal: openChatListModal, AlertChatListModal } = useChatListModal();

  // TODO: 재사용 가능성 있으면 빼는 게 좋음
  const routerLink = (linkId: string, status: string) => {
    if (status === 'ACCEPT' || status === 'SOULMATE') {
      router.push(`/chat-list/${linkId}`);
    } else return openModal('신호 대기중입니다!');
  ...
  return (
    <Page>
      {!chatList?.length ? (
        ...
        <ul className=" ">
          {chatList?.map((list, idx) => {
            // TODO: 아래 컴포넌트가 너무 비슷한데 하나로 만들 수 있지 않을까?
            if (getUid?.id === list.flirting_list.sender_uid.uid) {
              return (
                <li
                  key={idx}
...
// TODO: api 메소드들은 밖으로 빼기 -> 컴포넌트가 렌더링 될 때마다 생성함
async function fetchLastMessages(roomIds: string[]): Promise<LastMessageArrayType> {
  let lastMessageArray = [];
...
function ChatItem({
  ...
  const router = useRouter();
  const { openModal: openChatListModal } = useChatListModal();
  const routerLink = (linkId: string, status: string) => {
    if (status === 'ACCEPT' || status === 'SOULMATE') {
      router.push(`/chat-list/${linkId}`);
    } else return openModal('신호 대기중입니다!');
